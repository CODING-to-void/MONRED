(() => {
  ...
  function markLoadedImages(root = document) {
    root.querySelectorAll("img.blur-img, img.book-cover").forEach((image) => {
      ...
      image.addEventListener("error", () => {
        const card = image.closest("[data-pdf-title]");
        image.insertAdjacentHTML(
          "afterend",
          placeholderCover(card?.dataset.pdfTitle || image.alt || "Untitled", card?.dataset.pdfCreator || "MONRED"),
        );
        image.remove();
      }, { once: true });
    });
  ...
  function renderResults(docs, query = state.lastQuery) {
    ...
    if (state.sourceMode !== "annas-archive") {
      cards.push(
        ...docs.map((doc) => {
          ...
          return `
            <article class="release-card pdf-release-card" data-pdf-card="${escapeHtml(identifier)}" data-pdf-title="${escapeHtml(title)}" data-pdf-creator="${escapeHtml(creator)}">
    ...
    markLoadedImages(dom.pdfResultsGrid);
  }
  ...
  function openReader(identifier, fileName, title) {
    ...
    params.set("title", title || "PDF Reader");
    window.location.href = `./MONRED-reader.html?${params.toString()}`;
  }
